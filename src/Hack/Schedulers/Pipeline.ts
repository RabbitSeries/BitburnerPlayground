...
import { AwaitTasks, FindMaxHGWThreads, HackAligns, HostHandlers, OffsetAligns, ScheduleGrowTask, ScheduleWeakenTask, type HackThreadBoost } from "./ScheduleHelpers";
...
export async function Pipeline(ns: NS, target: string, preHandler = HostHandlers["KillAllExceptHome"], stop_token = new StopToken()) {
    ...
    while (!stop_token.is_stop_requested()) {
        ...
        const hackCondition = HackCondition(ns, target), weakenCondition = WeakenCondition(ns, target)
        if (weakenCondition) {
            ns.print("Pipeline Scheduler: Weakening")
            await AwaitTasks(ns, ScheduleWeakenTask(ns, servers, target, preHandler, () => { }));
            continue
        } else if (!hackCondition) {
            ns.print("Pipeline Scheduler: Growing")
            await AwaitTasks(ns, ScheduleGrowTask(ns, servers, target, preHandler, () => { }));
            continue
        }
        ...
        while (!stop_token.is_stop_requested()) {
            ...
            let pipeLineId = 0, newTask = false
            if (taskQ.length < pHeight) {
                ...
                if (now - taskQ[0].logicBegin >= aligned && running === null) {
                    ...
                    ns.print(`\tThis task was aligned to ${ns.tFormat(fixedAligned - delay, true)}}, ${ns.tFormat(fixedAligned, true)}]`)
                    ns.print(`\tTarget Money: ${ns.formatPercent(ns.getServerMoneyAvailable(target) / ns.getServerMaxMoney(target), 1)}`)
                    ns.print(`\tTarget Security: ${ns.formatPercent(ns.getServerMinSecurityLevel(target) / ns.getServerSecurityLevel(target), 1)}`)
                } else {
                    ns.print(`Task is runnning on host ${host}, pid ${pid}, remaining: ${ns.tFormat(now - logicBegin, true)}`)
                }
            }
            ...
            if (newTask) {
                ...
                const hPid = new HackMiner(ns, host, target, hThread, fixedAligns.HackTime - delay).run()
                const wPid1 = new WeakenMiner(ns, host, target, wThread1, fixedAligns.weaken1Time - 2 * delay / 3).run()
                const gPid = new GrowMiner(ns, host, target, gThread, fixedAligns.growTime - delay / 3).run()
                const wPid2 = new WeakenMiner(ns, host, target, wThread2, fixedAligns.weaken2Time).run()
                if (!hPid || !wPid1 || !gPid || !wPid2) {
                    ns.print(`Failed to launch on ${host}: ${pHosts[pipeLineId].launched} / ${pHosts[pipeLineId].count}`)
                    break
                }
                taskQ.push({ pid: wPid2, pipeLineId, logicBegin: now + offset, aligned: fixedAligned })
                ns.print(`\tNext segment running on ${wPid2}, pipeline height: ${taskQ.length}/${pHeight}`)
            }
            ns.print(`\twait: ${ns.tFormat(nextWait, true)}, per segment: ${ns.tFormat(pSegTime, true)}`)
