...
export function ServerInfoObject(ns: NS,
    rowId: number,
    host: string): Record<TableHeaderMeta, React.JSX.IntrinsicElements["td"]> {
    const formatNumber = ns.format.number
    const server = serverInfo(ns, host)
    const MoneyRate = CurrMoneyFlow.bind(ns)(host),
        pMoney = PotentialMoneyFlow.bind(ns)(host)
    return {
        "Rank": <td>{rowId} </td>,
        ...
        "Money": (
            <td>
                {formatNumber(server.moneyAvailable ?? 0,
                    1)} / {formatNumber(server.moneyMax ?? 0, 1)}
                {`(${server.moneyMax ? ns.format.percent(
        ...
        "Security": (
            <td>
                {formatNumber(server.minDifficulty ?? 0, 0)} /
                {formatNumber(server.hackDifficulty ?? 0, 1)}
            </td>
        ),
        "HWG Time/mins": (
            <tbody>
        ...
        "Current$/s": <td>{isNaN(MoneyRate) ? "NAN" :
            formatNumber(MoneyRate, 1)} </td>,
        "Potential$/s": <td>{isNaN(pMoney) ? "NAN" :
            formatNumber(pMoney, 1)} </td>,
        "RAM": (
            <td>{`${formatNumber(server.maxRam - server.ramUsed,
                2)}/${formatNumber(server.maxRam, 2)}`} </td>
        ),
        "Contracts": <td style={{ textAlign: "center" }}> {ns.ls(host,
            ".cct").length} </td>,
        "Cores": <td>{ns.getServer(host).cpuCores} </td>,
        // Add a key to this, so the diff algorithm won't recreate
        // new content for it, when reorders the table.
        "Actions": <Actions key={host} host={host} ns={ns} />
    }
}

export function ServerInfo({ ns, host, rowId }: { ns: NS,
                                                  host: string,
                                                  rowId: number }) {
    ...
    return (
        ...
        >
            {Object.values(content).map(node => {
