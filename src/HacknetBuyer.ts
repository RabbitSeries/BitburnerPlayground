...
export async function main(ns: NS) {
    ...
    ns.print(`Currently having ` +
        `${hacknet.numNodes()}/${hacknet.maxNumNodes()} nodes`)
    while (hacknet.numNodes() < 30) {
        const cost = hacknet.getPurchaseNodeCost()
        const wallet = ns.getServerMoneyAvailable("home")
        ns.print(`Next node with costs ` +
            `${ns.format.number(cost)}/${ns.format.number(wallet)}`)
        while (wallet < cost) {
            ns.print(`Nodes: ${ns.hacknet.numNodes()}`)
            const maxLevel = [...Array(ns.hacknet.numNodes())]
                .map(function (this: NS, _, i) {
                    return this.hacknet.getNodeStats(i).level
                }, ns)
                .reduce((init, v) => Math.max(init, v), 0)
            for (const id of
                [...Array<number>(hacknet.numNodes())].map((_, i) => i)) {
                upgradeLevelTo(ns, id, maxLevel + 1)
            }
            await ns.sleep(10000)
        }
        ns.print(`Buying ${hacknet.numNodes()}_th` +
        `/${hacknet.maxNumNodes()} node`)
