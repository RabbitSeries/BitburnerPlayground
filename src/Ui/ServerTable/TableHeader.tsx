...
export function TableHeader({ ns, setRanker }:
{ ns: NS, setRanker: (ranker: Comparator<string>) => void }) {
    ...
    const header = {
        ...
        "Security":RootAccessRank(ns)
            .thenSortBy(SecurityLevelRank(ns).compare),
        "HWG Time/mins": RootAccessRank(ns)
            .thenSortBy(HackTimeRank(ns).compare),
        "Growth": RootAccessRank(ns)
            .thenSortBy(Comparator.comparing(ns.getServerGrowth).compare),
        "Current$/s": RootAccessRank(ns)
            .thenSortBy(CurrentMoneyRateRank(ns).compare),
        "Potential$/s": RootAccessRank(ns)
            .thenSortBy(PotentialMoneyRank(ns).compare),
        "RAM": RootAccessRank(ns)
            .thenSortBy(Comparator.comparing(ns.getServerMaxRam).compare)
            .reversed(),
        "Contracts": Comparator.comparing<string>((host) =>
            ns.ls(host, ".cct").length).reversed(),
        "Cores": Comparator.sortBy<string>((a, b) =>
        ...
        <thead>
            <tr>
                {Object.entries(header).map(([n, r]) => (
                    <th style={{textAlign: "left"}} onClick={() => {
