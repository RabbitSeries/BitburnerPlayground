...
export function TryHacking(ns: NS, miner: IMiner, ...args: ScriptArg[]): number {
	...
	try {
		...
		if (freeRam < required) {
			...
			throw new Error(
				[
					`WARN: Free ram is not satisfied ${formated}`,
					`Try to run the command at ${miner.args.hostName}:`,
					`run ${scriptPath} -t ${+threadOptions} ${args.join(" ")}`
				].join("\n")
			)
		}
		...
		if (result === 0) {
			throw new Error(
				["ERROR: ", "Runing", scriptPath, "at", currentHost, "with ram", formated].join(" ")
			)
		}
		return result
	} catch (e) {
		ns.print(
			`ERROR: running ${scriptPath} on ${currentHost} hacking ${target} with threadOptions: ${threadOptions}`
		)
...
export function Scp(ns: NS, destination: string): boolean {
	return ns.scp(
		ns.ls("home").filter((path) => path.endsWith(".js")),
		destination
	)
}
