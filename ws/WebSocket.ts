...
import type { DeleteFile, DeleteFileResponse, GetDefinitionFile, GetDefinitionFileResponse, GetFileNames, GetFileNamesResponse, PushFile, PushFileResponse } from "./Messages.js"
...
const validateFilenames = (filenames: string[]) => filenames.filter(name => config.filters.has(extname(name)))
...
export async function getDefinitionFile(socket: WebSocket) {
    ...
    messageHandlers.set(defRequest.id, async (json) => {
...
export async function pushFile(filename: string, socket: WebSocket, server: string) {
    ...
    messageHandlers.set(filePush.id, (json) => {
        const { error, result } = json as PushFileResponse
        console.log({ Pushstat: `${error ?? ''}:${result}`, file: filePush.params!.filename, aka: filename, server, bytes: filePush.params!.content.length })
    })
...
export async function getFilenames(socket: WebSocket, server: string) {
    const filenames = new Promise<string[]>(((resolve, reject) => {
        ...
        messageHandlers.set(filenamesRequest.id, async (json) => {
...
export async function deleteFile(filename: string, socket: WebSocket, server: string) {
    ...
    messageHandlers.set(fileDeletion.id, (json) => {
        const { error, result } = json as DeleteFileResponse
        console.log({ Deletestat: `${error ?? ''}:${result}`, file: fileDeletion.params!.filename, aka: filename, server })
    })
    ...
    server.on("connection", async (socket, message) => {
        ...
        if (config.cleanServers) {
            await iterateServers(async (server) => {
                await getFilenames(socket, server).then(filenames => {
